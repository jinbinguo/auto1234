if exists (select 1 from sysobjects where name = 'RPT_VehicleSaleGatherByModel' and xtype='P')
	drop procedure RPT_VehicleSaleGatherByModel
go

create procedure RPT_VehicleSaleGatherByModel
	@beginDate date, --开始日期
	@endDate date, --结束日期
...
BEGIN
	...
	set @sql = 'insert into #tmpTable(FBrandName,FSeriesName,FModelName,
			FQty,FSaleAmt,FPreferentialAmt,
			FARAmount,FOptionalAmt,FSecondHandAmount,
			FARRealAmount,FReceiptBindingAmount)
		select e.FName FBrandName,d.FName FSeriesName,c.FName FModelName,
	...
	select FBrandName 品牌,FSeriesName 车系,FModelName 车型, 
