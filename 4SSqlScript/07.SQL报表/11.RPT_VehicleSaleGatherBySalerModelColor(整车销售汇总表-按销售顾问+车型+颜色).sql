if exists (select 1 from sysobjects where name = 'RPT_VehicleSaleGatherBySalerModelColor' and xtype='P')
	drop procedure RPT_VehicleSaleGatherBySalerModelColor
go

create procedure RPT_VehicleSaleGatherBySalerModelColor
	@beginDate date, --开始日期
	@endDate date, --结束日期
...
BEGIN
	...
	set @sql = 'insert into #tmpTable(FSaler,FBrandName,FSeriesName,FModelName,FColorName,
			FQty,FSaleAmt,FPreferentialAmt,
			FARAmount,FOptionalAmt,FSecondHandAmount,
			FARRealAmount,FReceiptBindingAmount)
		select f.FName FSaler,e.FName FBrandName,d.FName FSeriesName,c.FName FModelName,g.FColor FColorName,
		...
		left join T_ATS_SeriesColor g on g.FID=d.FID and g.FEntryID=b.FColorID 
		where a1.FMultiCheckStatus=''16''' + @filterSQL +
		'group by f.FName,e.FName,d.FName,c.FName,g.FColor'
	execute sp_executesql @sql
	select FSaler 销售顾问,FBrandName 品牌,FSeriesName 车系,FModelName 车型, FColorName 颜色,
